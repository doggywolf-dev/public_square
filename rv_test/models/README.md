# Обучение модели на детекцию трех классов (airplanes, birds и kites).

## Результат
[Сама модель] и та же модель, конвертированная в [ONNX].
...
Выбор был сделан исходя из большой скорости работы таких моделей, позволяющей использовать их при обработке видео в реальном времени. Недостатком этой модели является меньшая чувствительность к мелким объектам, ограниченное число предсказаний и относительно невысокая точность локализации.
...
Использованные скрипты находятся в директории [..\data_scripts](https://github.com/doggywolf-dev/public_square/tree/main/rv_test/data_scripts)

### Дополнительно размеченные данные
Вручную были выбраны из интернета и размечены на roboflow.com и добавлены:
- в train 37 изображений фона и мусорных объектов, по 35 изображений птиц, самолетов и воздушных змеев.
- в val 21 изображение фона и мусорных объектов, по 11 изображений птиц, самолетов и воздушных змеев.

### Полностью ручной пересбор тестового множества
Для объективной проверки качества модели был собран вручную и размечен на roboflow.com набор ground_truth, содержащий по 35 изображений птиц, самолетов, воздушных змеев и фона.

## Порядок обучения модели

Дообучение модели YOLOv8n производилось в три стадии

### 1. Быстрое базовое обучение на низком разрешении
10 эпох, Разрешение 320, без аугментации. Все гиперпараметры перечислены в конфиге [train_stage_0.py](https://github.com/doggywolf-dev/public_square/blob/main/rv_test/models/train_stage_0.py)

### 2. Обучение на низком разрешении с аугментацией данных
20 эпох, Разрешение 320, с базовой аугментацией. Гиперпараметры перечислены в конфиге [train_stage_1.py](https://github.com/doggywolf-dev/public_square/blob/main/rv_test/models/train_stage_1.py)

### 3. Обучение на высоком разрешении с усиленной аугментацией данных на расширенном наборе данных.
до 50 эпох, Разрешение 640, с усиленной аугментацией на расширенном ручной разметкой датасете. Гиперпараметры перечислены в конфиге [train_stage_2.py](https://github.com/doggywolf-dev/public_square/blob/main/rv_test/models/train_stage_2.py)

## Результаты на тестовом множестве.
Значения Precision, Recall, mAP50 и mAP50-95 для всего теста и для каждого класса отдельно выложены в директории [metrics](https://github.com/doggywolf-dev/public_square/tree/main/rv_test/models/metrics)
